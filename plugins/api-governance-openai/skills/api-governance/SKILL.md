...
<!--
  ...
  - Rule sentences: the canonical CodeRifts agent-host rule, published as the CODERIFTS_POLICY
    constant in @coderifts/sdk. `npm run validate:rule-provenance` holds the recorded carriers in
    this repository to it.
  - Tool names: ../../../../tools.wire.v1.json, pinned byte-for-byte to the live tools/list and
    checked by `npm run validate:tools-wire`.
  - MCP endpoint: ../../.mcp.json in this package.
  This file's trigger description is channel-tuned for OpenAI/Codex on purpose and is NOT a byte
  copy of the generated skill — do not regenerate it over the top.
-->
