...
This repository is the public entry for the hosted MCP server (https://app.coderifts.com/mcp). A researcher looking for where to report should start here.
...
The hosted MCP exposes three tools: preflight_change_set, verify_receipt, get_decision_details. GET /api/v1/public/preflight is analyze-only: there is no decision field, authorization_effect is NONE, and may_execute is false. That response is not permission.
...
A tool call that never reaches these three tools is invisible here. This repository does not wrap, replace, or intercept any other tool your agent holds. The runtime guarded table is @coderifts/agent-guard, not this package.
